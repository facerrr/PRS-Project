...
def fusion_plot(img=None, pred_boxes = None, names = None, conf=True, right_xy = None, left_xy = None,):
    font='Arial.ttf'
    annotator = Annotator(
            im=deepcopy(img),
            font=font,
...
def trash_detect_inference(img):
    global trash_result
    # time.sleep(3)
...
names = trash_model.names

img = cv2.imread("docs/test.png")
pose_result = pose_model(img)[0]
cv2.imwrite('docs/pose_res.png',pose_result.plot())
e_ws = get_wrist_elbow_point(pose_result.boxes,pose_result.keypoints)
font='Arial.ttf'
annotator = Annotator(
        im=deepcopy(pose_result.orig_img),
        font=font,
        example=names)
if e_ws is not None:
    # for e_w in e_ws:
    bcx,bcy = derivate_box_center(e_ws[0][0],e_ws[0][1],150)
    x1,y1,x2,y2= box((bcx,bcy),150)
    crop = pose_result.orig_img[y1:y2, x1:x2]
    cv2.imwrite('docs/crop.png',crop)
    trash_result = trash_model(crop)[0]
    ...
    cv2.imwrite('docs/test_fusion_result.png',annotator.result())
else:
    cv2.imwrite('docs/test_fusion_result.png',annotator.result())
